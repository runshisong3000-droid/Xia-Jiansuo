...
打包命令:
  python -m PyInstaller --clean --onefile --windowed --icon xia_jiansuo_avatar.ico --name 瞎检索 xia_jiansuo_fixed_v4.py
...
def extract_title(ref: str) -> str:
    ...
    # 中文文献优先用 GB/T/中文解析器，避免把作者、期刊或年份当题名
    if has_chinese(ref):
        parsed = parse_chinese_reference(ref)
        if parsed.get("title"):
            return parsed["title"]
...
class XiaJiansuoApp(tk.Tk):
    ...
    def _build_header(self):
        ...
        tk.Label(title_block, text="CrossRef · 中文解析 · APA 7 · BibTeX",
